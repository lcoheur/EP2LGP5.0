...
def compara_listas(elemento_frasico_regra_1, elemento_frasico_regra_2):
	...
	if len(elemento_frasico_regra_1) != len(elemento_frasico_regra_2):
		return False

	return elemento_frasico_regra_1 == elemento_frasico_regra_2
...
def estatistica_regras(elemento):
	"""
	...
	regras_pt, regras_lgp, tipo_regras = [], [], []

	
	with open('Regras_de_traducao/Regras_morfossintaticas/regras_de_'+elemento+'.csv', 'r') as file:
		lines = file.readlines()

		for line in lines[:-1]: #does not read the last empty line
			split_line = line.strip().split("\t")
			regras_pt.append(split_line[0])
			regras_lgp.append(split_line[1])

			if len(split_line) > 2:
				if split_line[2].startswith("N"):
					tipo_regras.append("NEG")
				elif split_line[2].startswith("I"):
					tipo_regras.append("INT")
				else:
					tipo_regras.append("CAN")
			else:
				tipo_regras.append("CAN")
	...
	for i in range(len(regras_pt)):
		...
		for j in range(len(regras_pt)):
			...
			if tipo_regras[i] == tipo_regras[j] and compara_regra(a,b,c,d):
...
def alinhamento(elemento_frasico_pt, elemento_frasico_lgp, opcao, tipo):
	...
	if alinhamento_wordNet_final:
		...
		while sim:
			...
			line_to_write = [" ".join(lexico_pt)," ".join(lexico_pt_lema), lexico_lgp, classes_lgp[0][:-1]]

			line_string = '\t'.join(line_to_write[:3])
			class_lgp = ''.join(line_to_write[3])

			with open('Dicionario/dicionario.csv', newline='') as file_read:
			...
			flag = True
			for line in lines:
				if line[0].startswith(line_string) or "?" in line_string or "/" in line_string or "?" in class_lgp or "/" in class_lgp:
					flag = False

			with open('Dicionario/dicionario.csv', 'a+') as f:
				csvw = csv.writer(f, delimiter='\t')
				if flag and m[3] >= 0.9: # test with 0.9
					if " ".join(lexico_pt) not in "[]/" and " ".join(lexico_pt_lema) not in "[]/" and lexico_lgp not in "[]/":
						csvw.writerow(line_to_write)
...
def valor_estatisticas(pt, lgp, elemento):
	...
	if pt and lgp:
		estatistica_suj = estatistica_regras(elemento)
