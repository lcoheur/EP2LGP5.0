...
def estrutura_sintatica_frase(dependencies_tags, pred_tags, dep_words, frase):
	...
	for index, item in enumerate(dependencies_tags):
		...
		if "obj" in item:
			estrutura.append("O")
		if "amod" in item and index < len(pred_tags) and pred_tags[index].startswith("V") and "ROOT" in dependencies_tags:
			if not (type(frase) is Frase_pt):
				estrutura.append("V")
				frase.set_classes_v(pred_tags[index])
				frase.set_classes_antes_v(dep_words[index], pred_tags[index])
				indice_verbo = index
		if "ROOT" in item and "cop" not in dependencies_tags and index < len(pred_tags) and pred_tags[index].startswith("V"):
			estrutura.append("V")
			indice_verbo = index
		if "ROOT" in dependencies_tags and index < len(pred_tags) and pred_tags[index].startswith("V"):
			estrutura.append("V")
			indice_verbo = index
		if "cop" in item and "ROOT" in dependencies_tags and index < len(pred_tags) and pred_tags[index].startswith("V"):
			estrutura.append("V")
			indice_verbo = index
...
def set_elementos(dependencies_tags, pred_tags, dep_words, frase):
	...
	for i, d in enumerate(dependencies_tags):
		...
		if "nsubj" in d:
			if len(pred_tags) > i and len(dep_words) > i:
		...
		if (d == "amod" or d=="case") and "ROOT" in dependencies_tags and indice_verbo!= dependencies_tags.index("ROOT") and i > indice_verbo:
			if len(pred_tags) > i and len(dep_words) > i:
				frase.set_classes_obj(pred_tags[i])
				frase.set_classes_antes_obj(dep_words[i], pred_tags[i])
				frase.set_indices_obj(i)

		if (d == "amod" or d=="case") and indice_subj==-1 and "ROOT" in dependencies_tags and indice_verbo!= dependencies_tags.index("ROOT") and i < indice_verbo:
			if len(pred_tags) > i and len(dep_words) > i:
				frase.set_classes_outros(pred_tags[i])
				frase.set_classes_antes_outro(dep_words[i], pred_tags[i])
				frase.set_indices_outros(i)

		if d == "ROOT" and len(pred_tags) > i and not pred_tags[i].startswith("V") and i < indice_verbo:
			if len(dep_words) > i:
				frase.set_classes_suj(pred_tags[i])
				frase.set_classes_antes_suj(dep_words[i], pred_tags[i])
				frase.set_indices_suj(i)
		...
		if "acl" in d and len(pred_tags) > i and pred_tags[i].startswith("V"):
			if len(dep_words) > i:
		...
		if (d == "ROOT" or d == "case") and "cop" not in dependencies_tags and len(pred_tags) > i and pred_tags[i].startswith("V"):
			if len(dep_words) > i:
				frase.set_classes_v(pred_tags[i])
				frase.set_classes_antes_v(dep_words[i], pred_tags[i])
				frase.set_indices_verbo(i)
		...
		if d == "xcomp" and len(pred_tags) > i and pred_tags[i].startswith("V"):
			if len(dep_words) > i:
		...
		if d == "amod" and "ROOT" in dependencies_tags and len(pred_tags) > i and pred_tags[i].startswith("V"):
			frase.set_indices_verbo(i)


		if d == "ROOT" and "cop" in dependencies_tags and len(pred_tags) > i and pred_tags[i].startswith("V"):
			if len(dep_words) > i:
				frase.set_classes_obj(pred_tags[i])
				frase.set_classes_antes_obj(dep_words[i], pred_tags[i])
				frase.set_indices_obj(i)
		...
		if (d == "amod" or d == "case") and "ROOT" in dependencies_tags and indice_verbo== dependencies_tags.index("ROOT"):
			if len(pred_tags) > i and len(dep_words) > i:
				frase.set_classes_outros(pred_tags[i])
				frase.set_classes_antes_outro(dep_words[i], pred_tags[i])
				frase.set_indices_outros(i)
