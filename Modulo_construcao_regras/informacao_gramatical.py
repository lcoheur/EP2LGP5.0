...
def estatistica_estrutura(tipo, estatisticas):
	"""
	...
	if str(tipo) == "CAN":
		with open('Regras_de_traducao/Regras_frasicas/can.csv', 'r') as file_can:
			lines = file_can.readlines()

			for line in lines: #does not read the last empty line
		...
		with open('Regras_de_traducao/Regras_frasicas/int.csv', 'r') as file_int:
			lines = file_int.readlines()
		...
		with open('Regras_de_traducao/Regras_frasicas/neg.csv', 'r') as file_neg:
			lines = file_neg.readlines()
		...
		with open('Regras_de_traducao/Regras_frasicas/excl.csv', 'r') as file_excl:
			lines = file_excl.readlines()
...
def main(ficheiro_html):
	...
	info, pt_sentences, lgp_sentences = parserELAN.main(ficheiro_html) #parse das informações do corpus (que estão num ficheiro html exportado do ELAN).
	...
	for i in info:
		...
		if i.get_tipo_frase() == "INT":
			...
			with open('Regras_de_traducao/Regras_frasicas/int.csv', 'a+') as f:
				if estrutura_pt and estrutura_lgp:
	...
	if os.path.exists('Regras_de_traducao/Regras_frasicas/can.csv'):
		estatisticas = estatistica_estrutura("CAN", estatisticas)
	if os.path.exists('Regras_de_traducao/Regras_frasicas/int.csv'):
		estatisticas = estatistica_estrutura("INT", estatisticas)
	if os.path.exists('Regras_de_traducao/Regras_frasicas/neg.csv'):
		estatisticas = estatistica_estrutura("NEG", estatisticas)
	if os.path.exists('Regras_de_traducao/Regras_frasicas/excl.csv'):
		estatisticas = estatistica_estrutura("EXCL", estatisticas)
	guarda_estatisticas(estatisticas)

	return info_todas_pt, info, pt_sentences, lgp_sentences
