...
class Frase_input:
	...
	def set_frase_sem_det(self, palavra, lemma, classe):
		palavra = [word.strip('“”') if '“' in word or '”' in word else word for word in palavra]
		palavra = [word for word in palavra if word != '']
	...
	def set_frase_sem_det_lemmas_verd(self, palavra, lemma, classe):
		palavra = [word.strip('“”') if '“' in word or '”' in word else word for word in palavra]
		palavra = [word for word in palavra if word != '']
		lemma = [word.strip('“”') if '“' in word or '”' in word else word for word in lemma]
		lemma = [word for word in lemma if word != '']
		for p in range(len(palavra)):
