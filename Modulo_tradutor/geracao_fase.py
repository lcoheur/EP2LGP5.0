...
def feminino(traducao, excepcoes, list_word_sign):
	"""
			...
			if palavra not in excepcoes:
				if palavra.lower()!=lema.lower():
					if classe.endswith("D"):
						if classe.startswith("NCFP") and palavra[:-5].lower() != lema[:-1].lower():
							glosa = "MULHER"
							glosa_lemma = "mulher " + lema + " " + "pequeno"
							list_word_sign.append((palavra, glosa_lemma))
							...
							if classe.endswith("D"):
								diminutivo = "PEQUENO"
								glosa_lemma = lema + " " + diminutivo.lower()
								list_word_sign.append((palavra, glosa_lemma))
						...
						if classe.startswith("NCFP") and palavra[:-5].lower() != lema[:-1].lower():
							glosa = "MULHER"
							glosa_lemma = "mulher " + lema + " " + "grande"
							list_word_sign.append((palavra, glosa_lemma))
							traducao.insert(indice, (glosa, glosa, "A"))
							traducao[indice + 1] = (lema, lema, classe + "_COMP")
				...
				if palavra.lower() != lema.lower():
					if classe.endswith("D"):
						...
						list_word_sign.append((palavra, excepcoes[palavra]))
						traducao[indice] = (excepcoes[palavra], lema, classe)
				else:
					list_word_sign.append((palavra, excepcoes[palavra]))
		indice += 1
...
def geracao(i, counter, exprFaciais, negativa_irregular, gestos_compostos, list_word_sign):
	"""
	...
	traducao_aux = i.traducao

	i.traducao = []

	print("traducao_aux: ", traducao_aux)
	for j in traducao_aux:
		if len(j) == 3:
			i.traducao.append(j)
	...
	feminino(i.traducao, excepcoes, list_word_sign)
	print("aqui2")
	print(i.traducao)
